...
const EventCreationChecker = async (req, res, next) => {
  if (req.headers.authorization) {
    ...
    try {
      ...
      const planDetails = await SubscriptionModel.aggregate([
        { $match: { _id: new mongoose.Types.ObjectId(plan) } },
        { $unwind: "$featurelist" },
        {
          $lookup: {
            from: "features",
            ...
            foreignField: "_id",
            pipeline: [{ $match: { name: "EventCreation", status: true } }],
            as: "featurelist",
          },
        },
      ...
      if (planDetails.length == 0) {
        return res.json({
          status: "error",
          message:
            "Feature Not Found in Subscription. Please Renew Subscription",
        });
      } else {
        next();
      }
      ...
      return res.json({
        status: "error",
        message: `Token Expired. Please Login Again ${error.message}`,
        redirect: "/user/login",
      });
    }
  } else {
...
module.exports = { EventCreationChecker };
