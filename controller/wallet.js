...
const subAmountinWallet = async (props) => {
    ...
    try {
        ...
        wallet[0].balance = wallet[0].balance - amount;
        await wallet[0].save()
        return { status: 'success', message: `Successfully Deducted Balance From Your Wallet` }
    } catch (error) {
        return { status: 'error', message: `Failed To Add Balance in the Wallet ${error.message}` }
    }
}
...
const addAmountInAdminWallet = async (props) => {
    ...
    try {
        ...
        if (transaction.status === "error") {
            ...
            await wallet[0].save()
            return { status: 'success', message: `Successfully Transfered Amount to Admin Account.` }
        }

    } catch (error) {
        return { status: 'error', message: `Failed To Transfer Amount To Admin Account ${error.message}` }
    }
}
...
const subAmountInAdminWallet = async (props) => {
    ...
    try {
        ...
        return { status: 'error', message: `Failed To Deduct Amount From Admin Account ${error.message}` }
    }
}
